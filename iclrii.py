"""End-to-end experiment runner (ICLR-II variant).

This script mirrors ``iclr.py`` but **does not** filter (A,B,x0) tuples by
density or identifiability margins. Instead, it:
1) builds the (A,B) ensemble via sim_regcomb_ctrb
2) keeps all (A,B) draws as-is
3) generates x0 using a user-specified scheme (sphere or sparse mask)
4) runs the remaining estimation + boxplot steps unchanged
"""
